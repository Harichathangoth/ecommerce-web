...
function CategoryPageContent({ params }: { params: { slug?: string[] } }) {
  ...
  const slugSegments = params.slug || ['mobiles', 'smartphones'];
  const mainCategory = slugSegments[0] || 'mobiles';
  const subCategory = slugSegments[1] || 'smartphones';

  // Capitalize title
  const formattedTitle = `All ${subCategory.charAt(0).toUpperCase() + subCategory.slice(1)}`;
  ...
  // Derive dynamic breadcrumbs from URL path slug
  const breadcrumbItems = slugSegments.map((segment, index) => {
    const isLast = index === slugSegments.length - 1;
    const path = `/category/${slugSegments.slice(0, index + 1).join('/')}`;
  ...
  return (
    <div className="container py-6">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <aside className="lg:col-span-3 bg-card border border-border rounded-xl p-5 space-y-6 shadow-sm">
          ...
          <div className="space-y-2.5">
            ...
            <div className="space-y-1.5 text-xs">
              {[
                { name: `All ${subCategory.charAt(0).toUpperCase() + subCategory.slice(1)}`, count: 120, active: true },
                { name: 'iPhone', count: 28 },
                { name: 'Samsung', count: 32 },
                { name: 'OnePlus', count: 14 },
                { name: 'Xiaomi', count: 18 },
                { name: 'Oppo', count: 12 },
                ...
                <div
                  ...
                  className={`flex items-center justify-between cursor-pointer py-1 px-2 rounded-md transition-colors ${
                    cat.active ? 'text-primary font-bold bg-primary/10' : 'text-muted-foreground hover:text-foreground'
                  }`}
                >
                  <span>{cat.name}</span>
        ...
        {/* Right Main Column: Breadcrumb + Title + Sort Bar + Product Grid + Pagination */}
        <main className="lg:col-span-9 space-y-5">
          ...
          {/* Line 2: Dynamic Category Headline */}
...
export default function CategoryPage({ params }: { params: { slug?: string[] } }) {
  return (
    <Suspense fallback={<div className="container py-12 text-center text-xs text-muted-foreground">Loading category...</div>}>
